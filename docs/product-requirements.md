...
| Term | Definition |
| :--- | :--- |
| **FEN** | *Forsyth-Edwards Notation*. A standard format for describing a particular board position of a chess game in a single text line. |
| **PGN** | *Portable Game Notation*. A standard plain text format for recording chess games (both moves and metadata) in a format readable by humans and software. |
| **SAN** | *Standard Algebraic Notation*. The standard format for recording chess moves (e.g. `Nf3`, `exd5`, `O-O`, `e8=Q#`). |
| **LAN / UCI** | *Long Algebraic Notation / Universal Chess Interface*. Notation used for engine communication representing source and destination squares (e.g. `e2e4`, `e7e8q`). |
